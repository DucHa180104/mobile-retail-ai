...
function HeroBanner({ featuredProduct }) {
  ...
  return (
    <section className="grid gap-3 lg:grid-cols-[2fr_0.95fr]">
      {/* Main hero card */}
      <article className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-blue-700 via-indigo-650 to-blue-900 px-7 py-8 text-white shadow-lg shadow-blue-900/20 sm:px-8 sm:py-9 animate-fade-in">
        {/* Background image */}
        <img
          src={imageUrl}
        ...
        <div className="relative max-w-lg">
          <span className="inline-flex items-center gap-1.5 rounded-full bg-orange-500 px-3 py-1 text-[10px] font-bold uppercase tracking-[0.18em] text-white shadow-sm animate-fade-in-up">
            <span className="h-1.5 w-1.5 rounded-full bg-orange-300 animate-pulse" />
            Nổi bật
          </span>
          <h1 className="mt-4 text-2xl font-black leading-tight tracking-tight sm:text-[2.5rem] sm:leading-[1.15] animate-fade-in-up animation-delay-75">
            {featuredProduct?.name || "Điện thoại cũ giá tốt"}
          </h1>
          <p className="mt-4 max-w-md text-sm leading-7 text-blue-100/90 animate-fade-in-up animation-delay-100">
            Máy đẹp, giá thật, mô tả đúng tình trạng thực tế. Kiểm tra kỹ ngoại hình,
            pin và chức năng trước khi chốt đơn.
          </p>

          <div className="mt-6 flex flex-wrap gap-3 animate-fade-in-up animation-delay-150">
            <a
              href="#new-arrivals"
          ...
          {featuredProduct?.price ? (
            <p className="mt-5 text-sm font-semibold text-blue-200 animate-fade-in-up animation-delay-200">
              Chỉ từ{" "}
